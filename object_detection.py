...
from openvino.runtime import AsyncInferQueue, CompiledModel, Core
...
def main():
    ...
    # Compile the model optimized for Throughput, can be toggled for Latency bu changing config value
    ...
    result_file = open(
        os.path.join(args.output_dir, job_id, "output.txt"), "w", encoding="utf-8"
    )
    ...
    with open(
        os.path.join(args.output_dir, job_id, "stats.json"), "w", encoding="utf-8"
    ) as file:
